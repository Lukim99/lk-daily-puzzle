...
export function GlasslessFrame({ state, puzzle, busy, onBack, onBuyHint, onComplete }: GlasslessFrameProps) {
  ...
  const memoryEdge = EDGES[initial.seed % EDGES.length]
  const cornerOrder = useMemo(() => {
    const ordered = rotate(CORNERS, (initial.seed >>> 3) % CORNERS.length)
    return ((initial.seed >>> 7) & 1) === 1 ? [ordered[0], ordered[3], ordered[2], ordered[1]] : ordered
  }, [initial.seed])
  const cursorPeriod = 820 + ((initial.seed >>> 9) % 5) * 110
  const warningEdge = EDGES[(initial.seed >>> 13) % EDGES.length]
  const holeDirection = EDGES[(initial.seed >>> 16) % EDGES.length]
  const gapEdge = EDGES[(EDGES.indexOf(warningEdge) + 2) % EDGES.length]
  const clipMinimum = 420 + ((initial.seed >>> 19) % 4) * 90
  ...
  useEffect(() => {
    const snapshot: SavedPuzzleState = {
      seed: initial.seed, phase, pollution, centerTouched, centerProof, edgeProof,
      stamps, inputProof, frameProof, foldProgress,
    }
    localStorage.setItem(storageKey, JSON.stringify(snapshot))
  }, [storageKey, initial.seed, phase, pollution, centerTouched, centerProof, edgeProof, stamps, inputProof, frameProof, foldProgress])
  ...
  useEffect(() => {
    ...
    if (phase === 1 && centerTouched && !centerProof) {
      ...
      setPhase(2)
      sound('proof')
    } else if (phase === 2 && memoryCycle && !edgeProof) {
      setEdgeProof(true)
      setCenterSuppressedEdge(false)
      setPhase(3)
      sound('cool')
    }
  }, [pollution, centerSuppressedEdge, phase, centerTouched, centerProof, memoryCycle, edgeProof])

  useEffect(() => {
    if (!dragging?.corner) return
    const timer = window.setInterval(() => {
      if (Date.now() - dragging.enteredAt <= clipMaximum) return
  ...
  const proof = useMemo<InteractiveProof>(() => ({
    ...
    frame_not_closed: foldProgress < 96,
  ...
  const moveMoth = (event: ReactPointerEvent) => {
    ...
    if (corner !== dragging.corner) {
      if (dragging.corner && !corner) finishCornerPass(dragging.corner, Date.now() - dragging.enteredAt)
  ...
  const startMothDrag = (id: number, event: ReactPointerEvent<HTMLButtonElement>) => {
    ...
    setDragging({ id, corner: getCorner(positions[id]), enteredAt: Date.now() })
    markAction(false)
  }
  ...
  const tryBreakFocus = () => {
    ...
    const cursorIsDark = elapsed % cursorPeriod > cursorPeriod * .52
    inputRef.current?.blur()
    setCursorFocused(false)
    if (!typedDuringFocus.current && cursorIsDark && pollution === 0) {
      setInputProof(true)
      setPhase(5)
      sound('proof')
    }
  ...
  const stagePointerUp = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    setPointerBlank(true)
    setLastInputBlank(true)
    setLastAction(Date.now())
  }
  ...
  return (
    <main className={`glassless-case phase-${phase} pollution-${pollutionLevel} ${typedPulse ? 'typed-flash' : ''} ${success ? 'is-success' : ''} ${frameFailure ? 'frame-failed' : ''}`}
      style={{
        '--pollution': pollution,
      ...
      <div ref={stageRef} className={`gf-stage warning-${warningEdge} hole-${holeDirection} gap-${gapEdge} ${expectedEdgeClass}`}
        ...
        {EDGES.map((edge) => (
          <div key={edge} className={`gf-passive gf-stain stain-${edge} ${centerProof && edge === memoryEdge ? 'has-center-scar' : ''} ${edgeProof && edge === memoryEdge ? 'has-memory' : ''}`}><i /></div>
        ))}
        {CORNERS.map((corner) => (
        ...
        <button className={`gf-glass ${centerProof ? 'is-disproved' : ''} ${inputProof ? 'has-void' : ''}`}
          onPointerDown={touchCenter} aria-label='검은 유리판'>
          <span className='gf-glass-shadow' /><span className='gf-light' /><span className='gf-cracks' />
        </button>

        {positions.map((position, index) => (
          <button key={index} className={`gf-moth moth-${index} ${dragging?.id === index ? 'is-dragging' : ''}`}
        ...
        {phase === 4 && (
          <div className={`gf-mouth ${cursorFocused ? 'is-focused' : ''} ${typedPulse ? 'typed' : ''}`}>
